...
public class GetCarParkAvailability {
	...
	private List<CarParkAvailability> generateCarParkAvailabilityList(AvailabilityResponse availabilityResponse) {
		return availabilityResponse.items().parallelStream().flatMap(item -> item.carParkData().stream())
				.map(this::mapToCarParkInfo).toList();
	}

	private CarParkAvailability mapToCarParkInfo(AvailabilityResponse.CarParkData carParkData) {
		return CarParkAvailability.builder().carParkNumber(carParkData.carParkNumber())
				.updateDatetime(carParkData.updateDatetime()).totalLots(carParkData.carParkInfo().get(0).totalLots())
				.lotsAvailable(carParkData.carParkInfo().get(0).lotsAvailable()).build();
	}
}
