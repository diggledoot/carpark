...
public class GetNearestAvailableCarParks {
	...
	public List<NearestCarPark> execute(Double latitude, Double longitude, Integer rawPage, Integer rawPerPage) {
		...
		List<CarParkAvailability> availableCarParks = carParkAvailabilityRepository
		...
		// Build response
		return nearestAvailableCarParks.stream().map(carParkInfo -> {
			CarParkAvailability carParkAvailability = getCarParkAvailabilityByNearestCarParkInfo(availableCarParks,
					carParkInfo);
	...
	private List<CarParkInfo> getNearestAvailableCarParks(List<CarParkAvailability> availableCarParks,
			List<CarParkInfo> nearestCarParks) {
		// Convert available car park numbers to set for quick diff check
		Set<String> availableCarParkNumbers = availableCarParks.stream().map(CarParkAvailability::getCarParkNumber)
				.collect(Collectors.toSet());

		// Get available nearest car parks
		return nearestCarParks.stream().filter(carPark -> availableCarParkNumbers.contains(carPark.getCarParkNo()))
				.toList();
	}

	private NearestCarPark buildNearestCarPark(CarParkAvailability carParkAvailability, CarParkInfo carParkInfo) {
		return NearestCarPark.builder().address(carParkInfo.getAddress()).latitude(carParkInfo.getLatitude())
				.longitude(carParkInfo.getLongitude()).totalLots(carParkAvailability.getTotalLots())
				.lotsAvailable(carParkAvailability.getLotsAvailable()).build();
	}

	private CarParkAvailability getCarParkAvailabilityByNearestCarParkInfo(List<CarParkAvailability> availableCarParks,
			CarParkInfo carParkInfo) {
		return availableCarParks.stream().filter(
				carParkAvailability -> carParkAvailability.getCarParkNumber().contentEquals(carParkInfo.getCarParkNo()))
				.toList().get(0);
	}

	private String generateNearestCarParkNumbersString(List<CarParkInfo> carParkInfos) {
		return carParkInfos.stream().map(carParkInfo -> String.format("'%s'", carParkInfo.getCarParkNo()))
