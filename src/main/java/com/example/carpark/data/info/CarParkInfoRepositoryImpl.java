...
public class CarParkInfoRepositoryImpl implements CarParkInfoRepository {
	...
	public void ingest(CarParkInfo carParkInfo) {
		String sql = """
				...
				""";
		jdbcTemplate.update(sql, carParkInfo.getCarParkNo(), carParkInfo.getAddress(), carParkInfo.getLatitude(),
				carParkInfo.getLongitude());
	}

	@Override
	public List<CarParkInfo> nearestCarPark(double latitude, double longitude, int offset, int limit) {
		// Application of Haversine formula to calculate distance between two points on
		// a sphere
		...
		String sql = """
				...
				OFFSET ?
				""";
		return jdbcTemplate.query(sql, carParkInfoRowMapper, EarthRadius.KILOMETER.getRadius(), latitude, latitude,
				longitude, limit, offset);
	}
}
